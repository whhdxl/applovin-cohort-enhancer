...
为 AppLovin Ads 的 Cohort 报表增加计算列和颜色设置的桌面浏览器 WebExtension。当前版本 `0.1.6`，无第三方运行时依赖。
...
默认开启「跟随报表自动展示重点指标」：显示当前可计算的 D0／D1／D3／D7／D14／D28 基础指标及 ROAS 增长系数；RPD 增长系数默认不重复开启。手动取消的列会记住。可关闭自动模式，完全手选。旧版默认六列配置自动升级；已有自定义选择保留手动模式，可在面板开启自动模式。

只显示当前有计算路径的列，未定制时增强列追加在原生列之后，向右滚动查看。增强列表头右边界可拖拽调宽（96–480px），双击恢复该指标默认宽度（基础指标 116px，增长系数 132px）；聚焦手柄后可用左右方向键微调、Home 恢复。
